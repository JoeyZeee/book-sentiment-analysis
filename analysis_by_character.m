...
lexicon_files = ["NRC_VAD_arousal.csv", "NRC_VAD_dominance.csv", "NRC_VAD_valence.csv"];
...
for lexicon=1:length(lexicon_files)
    lexicon_tables{lexicon} = readtable("lexicons/"+lexicon_files(lexicon));
...
for lexicon=1:length(lexicon_files)
    ...
    legend(character_labels)
    title("Analysis of " + lexicon_files(lexicon), 'Interpreter', 'none');
end
%end